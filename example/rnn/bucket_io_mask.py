...
def default_read_content(path):
    with open(path) as ins:
        ...
        # add special tokens
        content = ['<s>' + line.replace('\n', '</s>') for line in content]
        return content


def default_build_vocab(path):
    content = default_read_content(path)
    word_list = chain(*[line.split(' ') for line in content])
    # 0 is left for <s>, 1 is left for unk
    idx = 2
    the_vocab = OrderedDict()
    ...
    for word in word_list:
        ...
        if not word in the_vocab:
            ...
            idx += 1
    print len(the_vocab)
    # the_vocab['</s>'] = len(the_vocab)
    with open("./vocab","w") as f:
        for w, i in the_vocab.iteritems():
            f.write("%s\t%d\n"%(w,i))
    return the_vocab
...
class BucketSentenceIter(mx.io.DataIter):
    def __init__(self, path, vocab, num_buckets, batch_size,
        ...
        sentences = self.read_content(path)
        ...
        self.provide_data = [('data', (batch_size, self.default_bucket_key)),('mask', (batch_size, self.default_bucket_key))] + init_states
    ...
    def make_data_iter_plan(self):
        ...
        for i_bucket in range(len(self.data)):
            ...
            mask = np.zeros((self.batch_size, self.maxlen_per_bucket[i_bucket]))
    ...
    def __iter__(self):
        ...
        for i_bucket in self.bucket_plan:
            # don't forget to clear data buffer
            self.data_buffer[i_bucket] *= 0.
            self.label_buffer[i_bucket] *= 0.
            self.mask_buffer[i_bucket] *= 0.
            ...
            mask[:] = np.asarray(data > 0, dtype=int)
